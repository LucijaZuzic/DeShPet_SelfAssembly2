...
from utils import PATH_TO_NAME, AP_DATA_PATH, SP_DATA_PATH, AP_SP_DATA_PATH, TSNE_SP_DATA_PATH, TSNE_AP_SP_DATA_PATH, seed_h5_and_png
...
if len(sys.argv) > 1 and len(sys.argv[1]) <= MAX_LEN: 
    ...
    SA, NSA = load_data_SA(some_path, dict_peptides, names, offset, properties, masking_value)
    all_data, all_labels = merge_data(some_path, SA, NSA) 
    print('encoded')
        
    best_model_file, best_model_image = seed_h5_and_png(some_path)
    
    model_predictions_peptides = model_predict(some_path, all_data, all_labels, best_model_file, best_model, MAX_BATCH_SIZE, names)[:-1]
