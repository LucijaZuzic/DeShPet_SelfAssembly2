...
from utils import set_seed, final_log_name, final_dir, seed_h5_and_png, seed_loss_txt_name, seed_acc_txt_name, seed_res_name, seed_loss_png_name, seed_acc_png_name, DATA_PATH, AP_DATA_PATH, SP_DATA_PATH, AP_SP_DATA_PATH, TSNE_SP_DATA_PATH, TSNE_AP_SP_DATA_PATH, PATH_TO_NAME
...
for some_path in path_list:
        ...
        SA_data = np.load(DATA_PATH + 'data_SA_no_AI_updated.npy', allow_pickle = True).item()
        ...
        if not os.path.exists(final_dir(some_path).replace("final", "final_no_AI")):
                os.makedirs(final_dir(some_path).replace("final", "final_no_AI"))

        # Write output to file
        sys.stdout = open(final_log_name(some_path).replace("final", "final_no_AI"), "w", encoding="utf-8")

        # Save model to correct file based on number of fold
        model_file, model_picture = seed_h5_and_png(some_path)
        model_file = model_file.replace("final", "final_no_AI")
        model_picture = model_picture.replace("final", "final_no_AI")
        ...
        other_output = open(seed_res_name(some_path).replace("final", "final_no_AI"), "w", encoding="utf-8")
        other_output.write(
        ...
        other_output = open(seed_acc_txt_name(some_path).replace("final", "final_no_AI"), "w", encoding="utf-8")
        other_output.write(str(accuracy))
        other_output.write("\n")
        other_output.close()

        other_output = open(seed_loss_txt_name(some_path).replace("final", "final_no_AI"), "w", encoding="utf-8")
        ...
        plt.title(
	    "Final model no AI " + PATH_TO_NAME[some_path] + "\n Accuracy"
	)
        plt.ylabel("Accuracy")
        plt.xlabel("Epoch")
        plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=2)
        plt.savefig(seed_acc_png_name(some_path).replace("final", "final_no_AI"), bbox_inches="tight")
        plt.close()
        ...
        plt.xlabel("Epoch")
        plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=2)
        plt.savefig(seed_loss_png_name(some_path).replace("final", "final_no_AI"), bbox_inches="tight")
        plt.close()
