...
for some_path in path_list:
    ...
    fileopen = open(predictions_longest_name(some_path).replace("longest", "genetic"), "r")
    ...
    plt.savefig(scatter_name_long(some_path).replace("long", "genetic").replace(".png", "") + ".pdf", bbox_inches="tight")
    plt.close()
    new_dictio = {"Predicted self-assembly probability": predictions, "AP": actual_AP, "Regression": mymodel}
    df_new = pd.DataFrame(new_dictio)
    df_new.to_csv(scatter_name_long(some_path).replace("long", "genetic").replace(".png", "") + ".csv", index = False)
