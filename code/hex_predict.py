...
for some_path in path_list:
	...
	SA_AP, NSA_AP = load_data_SA(some_path, dict_hex, names, offset, properties, masking_value)
	all_data_AP, all_labels_AP = merge_data(some_path, SA_AP, NSA_AP) 

	best_model = ""
	model_file, model_picture = seed_h5_and_png(some_path)
	model_predictions = model_predict(some_path, all_data_AP, all_labels_AP, model_file, best_model, MAX_BATCH_SIZE, names)
