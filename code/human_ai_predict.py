...
for some_path in path_list:
    ...
    SA_human, NSA_human = load_data_SA(
        some_path, dict_human, names, offset, properties, masking_value
    )
    all_data_human, all_labels_human = merge_data(some_path, SA_human, NSA_human)

    SA_AI, NSA_AI = load_data_SA(
        some_path, dict_AI, names, offset, properties, masking_value
    )
    ...
    model_predictions_human = model_predict(
        some_path,
        all_data_human,
        all_labels_human,
        model_file_no_human,
    ...
    model_predictions_AI = model_predict(
        some_path,
        all_data_AI,
        all_labels_AI,
        model_file_no_AI,
    ...
    filehuman = open(
        predictions_hex_name(some_path)
        .replace("final", "final_no_human")
        .replace("hex", "human"),
        "w",
    )
