...
def load_data_AP(name="AP", offset=1):
    # Load AP scores.
    amino_acids_AP = np.load(
        DATA_PATH + "amino_acids_" + name + ".npy", allow_pickle=True
    ).item()
    dipeptides_AP = np.load(
        DATA_PATH + "dipeptides_" + name + ".npy", allow_pickle=True
    ).item()
    tripeptides_AP = np.load(
        DATA_PATH + "tripeptides_" + name + ".npy", allow_pickle=True
    ).item()
...
def load_data_SA(
    ...
    if path_used == SP_DATA_PATH:
        return load_data_SA_SP(
            SA_data, names, offset, properties_to_include, masking_value
        )
    if path_used == AP_SP_DATA_PATH:
        return load_data_SA_AP_SP(
            SA_data, names, offset, properties_to_include, masking_value
        )
...
def load_data_SA_SP(
    SA_data, names=["AP"], offset=1, properties_to_include=[], masking_value=2
):
    sequences, labels = read_from_npy_SA(SA_data)
...
def load_data_SA_TSNE(SA_data, names=["AP"], offset=1, masking_value=2):
    ...
    for index in range(len(sequences)):
        ...
        feature_dict_1 = np.load(
            DATA_PATH + "amino_acids_NEW1.npy", allow_pickle=True
        ).item()
        feature_dict_2 = np.load(
            DATA_PATH + "amino_acids_NEW2.npy", allow_pickle=True
        ).item()
        feature_dict_3 = np.load(
            DATA_PATH + "amino_acids_NEW3.npy", allow_pickle=True
        ).item()
