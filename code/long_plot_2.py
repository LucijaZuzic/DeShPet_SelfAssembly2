...
for val in duplicate_list:
    ...
    actual_AP[ix] = min(actual_AP[ix], duplicate_AP_list[ix_dup])
...
for some_path in path_list:
    ...
    plt.savefig(scatter_name_long(some_path).replace("long", "longmin"), bbox_inches="tight")
    plt.savefig(scatter_name_long(some_path).replace("long", "longmin").replace(".png", "") + ".svg", bbox_inches="tight")
    plt.savefig(scatter_name_long(some_path).replace("long", "longmin").replace(".png", "") + ".pdf", bbox_inches="tight")
    plt.close()
    new_dictio = {"Predicted self-assembly probability": predictions, "AP": actual_AP, "Regression": mymodel}
    df_new = pd.DataFrame(new_dictio)
    df_new.to_csv(scatter_name_long(some_path).replace("long", "longmin").replace(".png", "") + ".csv", index = False)
