...
for some_path in paths:
    ...
    for val in vals_in_lines:
        ...
        if val == "Loss = ":
            print(
                val.replace(" = ", "")
                + doubleArrayToTable(lines_dict[val], sd_dict[val], False, False)
            )
            continue
        if val == "Accuracy = ":
            print(
                val.replace(" = ", "")
                + doubleArrayToTable(lines_dict[val], sd_dict[val], False, True)
            )
            continue
        else:
            if val.find(":") == -1:
                print(
                    val.replace(" = ", "")
                    + arrayToTable(lines_dict[val], False, True, False)
                )
            else:
                print(
