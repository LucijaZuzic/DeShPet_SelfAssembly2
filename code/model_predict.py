...
def model_predict(path_used, test_data, test_labels, best_model_file, best_model, used_batch_size = 600, names = ["AP"]):
    # Load the best model.
    if best_model_file != "":
...
def common_designed_peptides(test_number, path_used, model_file, model, used_batch_size = 600, names = ["AP"], offset = 1, properties = [], masking_value = 2, thr = 0.5):
    ...
    model_predictions = model_predict(path_used, all_data, all_labels, model_file, model, used_batch_size, names)
...
def generate_predictions(model, test_number, path_used, test_data, test_labels, used_batch_size = 600, names = ["AP"], offset = 1, properties = [], masking_value = 2, thr = 0.5):
    model_predictions = model_predict(path_used, test_data, test_labels, "", model, used_batch_size, names)
    ...
    common_designed_peptides(test_number, path_used, "", model, used_batch_size, names, offset, properties, masking_value, thr)
