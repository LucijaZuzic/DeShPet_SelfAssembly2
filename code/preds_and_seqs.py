...
def read_all_model_predictions(
    some_path, min_test_number, max_test_number, final_model_type, iteration, some_seed
):
    all_predictions = []
    all_labels = []
    all_sequences = []
    for test_number in range(min_test_number, max_test_number + 1):
        predictions, labels, sequences = read_one_prediction(
            some_path, test_number, final_model_type, iteration, some_seed
        )
...
for some_path in paths:
    ...
    for seed in seed_list:
        set_seed(seed)
        all_predictions, all_labels, all_sequences = read_all_model_predictions(
            some_path, 1, 5, "weak", 1, seed
        )
        ...
        for label in all_labels:
            ...
            all_model_types.append(
                PATH_TO_NAME[some_path].replace("SP and AP", "Hybrid AP-SP")
            )
            aa_model_types.append(
                PATH_TO_NAME[some_path].replace("SP and AP", "Hybrid AP-SP")
            )

    dict_some[PATH_TO_NAME[some_path]] = {
        "sequences": all_seqs,
...
for i in range(5):
    ...
    write_all += write_str
    file_output = open(
        "../data/sequence_pred_seed_" + str(seed_list[i]) + ".csv",
        "w",
        encoding="utf-8",
    )
...
d = {
    "Predicted self assembly probability": aa_preds,
    "Self assembly status": aa_labels_new,
    "Model": aa_model_types,
}
